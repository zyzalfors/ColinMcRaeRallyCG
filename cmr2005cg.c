...
bool isplatform(char text[]) {
 for(size_t i = 0; i < strlen(text); i++) {
  text[i] = tolower(text[i]);
 }
 for(int i = 0; i < 4; i++) {
  if(strcmp(text, platforms[i]) == 0) {
   return true;
  }
 }
 return false;
...
void gencode(char code[], int32_t array1[], int32_t array2[], int32_t accesscode, int32_t cheatid) {
 int32_t cheatidmagic = 0x13CB5B * cheatid % 0x26DD;
 int32_t accesscodemagic = (accesscode % 0x3E8) ^ (0x20B9 * cheatidmagic);
 int32_t seed1 = calcseed(accesscodemagic % 0x9AD);
 int32_t seed2 = calcseed(((accesscodemagic ^ 0x114CF1) * ((0x41B * cheatidmagic) ^ ((accesscode / 0x3E8) % 0x3E8))) % 0x91D);
 int32_t buffer[6];
...
int main(int argc, char* argv[]) {
 ...
 if(strcmp(platform, "pc") == 0) {
  int32_t arr1[10] = {331, 1259, 241, 109, 521, 853, 71, 719, 941, 269};
  int32_t arr2[10] = {72481, 180307, 130241, 392827, 421019, 949147, 32801, 1296649, 91249, 639679};
  memcpy(array1, arr1, 10 * sizeof(int32_t));
  memcpy(array2, arr2, 10 * sizeof(int32_t));
 }
 else if(strcmp(platform, "ps2") == 0 || strcmp(platform, "xbox") == 0) {
  int32_t arr1[10] = {859, 773, 151, 47, 487, 211, 617, 131, 947, 313};
  int32_t arr2[10] = {69119, 67783, 70271, 77929, 238099, 148151, 472751, 818963, 1195489, 839381};
  memcpy(array1, arr1, 10 * sizeof(int32_t));
  memcpy(array2, arr2, 10 * sizeof(int32_t));
 }
 else if(strcmp(platform, "psp") == 0) {
  int32_t arr1[10] = {743, 1663, 227, 991, 443, 89, 571, 199, 1373, 601};
  int32_t arr2[10] = {35491, 783019, 1116491, 591319, 194591, 37369, 822839, 86083, 354661, 99809};
  memcpy(array1, arr1, 10 * sizeof(int32_t));
  memcpy(array2, arr2, 10 * sizeof(int32_t));
 }
